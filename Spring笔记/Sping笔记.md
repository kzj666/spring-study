...
2. 假设我们要使用有参构造创建对象
   ...
   2. 通过类型赋值
      ...
      <!--第二种方式：通过类型创建，不建议使用-->
      <bean id="user" class="com.kk.pojo.User">
          <constructor-arg type="java.lang.String" value="康梓健"/>
      </bean>
      ```
   ...
   3. 通过属性名赋值
      ...
      <!--第三种，直接通过参数名实现赋值-->
      <bean id="user" class="com.kk.pojo.User">
          <constructor-arg name="name" value="康梓健"/>
      </bean>
      ```
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <bean id="user" class="com.kk.pojo.User" p:name="康梓健" p:age="18"/>

    <!--c命名空间注入，通过构造器注入：construct-args-->
    <bean id="user2" class="com.kk.pojo.User" c:age="18" c:name="康梓健"/>
</beans>
```
...
1. 单例模式（Spring默认机制）
   ...
   ```xml
       <bean id="user2" class="com.kk.pojo.User" c:age="18" c:name="康梓健" scope="singleton"/>
   ```
...
2. 原型模式:每次从容器中get的时候，都会产生一个新对象
   ...
   ```xml
       <bean id="user2" class="com.kk.pojo.User" c:age="18" c:name="康梓健" scope="prototype"/>
...
```xml
    <bean id="cat" class="com.kk.pojo.Cat"/>
...
2. 属性如何注入
   ...
   @Component
   public class User {
       //相当于 <property name="name" value="康梓健"></property>
